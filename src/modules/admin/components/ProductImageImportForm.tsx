...
export function ProductImageImportForm() {
  const [summary, setSummary] = React.useState<ImageImportSummary | null>(null);
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res = await importProductImagesAction(fd);
      setSummary(res);
      ...
      >
        <div className="flex flex-wrap items-end gap-3">
          <div className="min-w-[240px] flex-1 space-y-1">
            <label htmlFor="image-csv-file" className="text-sm font-medium">
              CSV file
            </label>
            <input
              id="image-csv-file"
...
function SummaryPanel({ summary }: { summary: ImageImportSummary }) {
  if (!summary.ok) {
    return (
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        ...
        <Stat label="Updated" value={summary.updated} tone="success" />
...
function RowIcon({
  ...
  status: ImageImportSummary["rows"][number]["status"];
}) {
